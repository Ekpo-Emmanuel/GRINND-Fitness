...
export const createWorkout = mutation({
  args: {
    ...
    cardio: v.optional(v.array(v.object({
      id: v.string(),
      name: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.insert('workouts', {
      ...
      ...(args.cardio ? { cardio: args.cardio } : {}),
      createdAt: now,
      updatedAt: now,
    });
  },
...
export const updateWorkout = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.cardio !== undefined) update.cardio = args.cardio;
    return await ctx.db.patch(args.workoutId, update);
  },
});
